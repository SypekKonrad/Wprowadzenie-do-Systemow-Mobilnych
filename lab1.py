...
c = 2147483647
aktualne_ziarno = 1
# x1 = math.sqrt(-2 * math.log()) * math.cos(2 * math.pi)
# x2 = math.sqrt(-2 * math.log()) * math.sin(2 * math.pi)
...
plot.figure(1)
plot.title('rozklad poissona (lambda=5)')
plot.hist(dane_poisson, bins=15, edgecolor='black')
plot.xlabel('wartosc')
plot.ylabel('ilosc wystapien')

plot.figure(2)
plot.title('rozklad normalny (mu=10, sigma=2)')
